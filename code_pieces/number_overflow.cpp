...
public:
    ...
    double calculateCompoundInterest(double principal, double annualRate, int years) {
        ...
        int principalCents = static_cast<int>(std::round(principal * PRECISION_MULTIPLIER));
        double dailyRate = annualRate / 365.0;
        int days = years * 365;

        // Use the compound interest formula: A = P(1 + r)^n
        int amount = principalCents;
        for (int day = 0; day < days; ++day) {
            amount = static_cast<int>(std::round(amount * (1 + dailyRate)));
        }
    ...
    double calculatePortfolioValue(const std::vector<double>& transactions) {
        ...
        for (const double transaction : transactions) {
            ...
            int cents = static_cast<int>(std::round(transaction * PRECISION_MULTIPLIER));
            totalCents += cents;
        }
...
int main() {
    ...
    double investment = calculator.calculateCompoundInterest(
        10000000,
        0.08,
        30
    );
